//##########################
// TNPC Safe Test - Triangular Waveform with True Dead-time
// duty_P = 0.49, duty_N = 0.49, dead-time = 0.02 (200ns)
// 50kHz, only GA and GB active, G0 disabled
//##########################
...
#define CARRIER     1000
#define CMP_L       490     // 49% positive
#define CMP_H       510     // 51% (1 - 49% negative)
...
int main(void) {
    ...
    // GPIO mux: only GPIO6 and GPIO11 (GA and GB)
    // GPIO0 and GPIO2 set as GPIO output LOW (G0 disabled)
    EALLOW;
    ...
    EDIS;
    EPwm4Regs.TBCTL.bit.SWFSYNC = 1;  // sync EPWM4 and EPWM6
...
void ConfigureEPWM_Triangular(void)
{
    ...
    EPwm4Regs.TBCTL.bit.PHSEN    = TB_DISABLE;  // master
